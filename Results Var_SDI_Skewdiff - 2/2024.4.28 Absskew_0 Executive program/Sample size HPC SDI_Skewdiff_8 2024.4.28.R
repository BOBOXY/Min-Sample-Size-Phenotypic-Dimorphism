...
# library(car)
# library(devtools)
library("diptest")#unimodality tests(uesd in Jordan C. Mallon 2017)(Maechler 2015)
# library(gridExtra)
# library(multimode)
# library(psych)
# library(readr)
...
# showtext font
# font.paths()
# font_all <- font_files()#纭璺緞涓嬪彲鐢ㄥ瓧浣撯?濆彲閫氳繃font_files()鍑芥暟鏌ョ湅
# font_add(family="ARL",
...
# Cram茅r-von Mises test statistic
...
# Cram茅r-von Mises test (Hall and York, 2001)
...
excessmassex=function(data,mod0=1){
  ...
  if(mod0==1){
    ...
    minimos=rcpp_arma_minimos(minimos,Aord, mod0, ndata, iniint, finint, large)
    ...
    # print('for k2 start')
    # print(timeBegin)
    # print(difftime(timeEnd,timeBegin,units="secs"))
...
diffem_0=function(PC,distC,ndata,lambda,mod0){
  ...
  for(modtemp in 1:mod0){
    ...
    for(lm in 1:modtemp){
      ...
      if(diff(elememl1[[lm]])<0){
        ...
        #distC2a<-rcpp_distC2a(distC,ndata,elememl1[[lm]][1],elememl1[[lm]][2])
    ...
    for(lm in 1:modtemp){
      #distC2b<-rcpp_distC2b(distC,distC2b,ndata,elememl1[[lm]][1],elememl1[[lm]][2])
      distC2b[((row(distC)+col(distC)-1)>=elememl1[[lm]][2])&(col(distC)<=elememl1[[lm]][1])]=NA		
    }  
    
    #elv=as.vector(unlist(elememl1))####
    #distC2b<-rcpp_distC2b_N(distC,ndata,elv,modtemp)#####
...
diffem=function(PC,distC,ndata,lambda,mod0){
...
excessmassapp=function(data,mod0=2,gridsize=c(20,20)){
  ...
  # emval1_2[lit]=diffem_0(PC,distC,ndata,lambdas[lit],mod0)
  
  # emval1[lit]=diffem(PC,distC,ndata,lambdas[lit],mod0)
  
  # emval1[lit]=rcpp_diffem(PC,distC,ndata,lambdas[lit],mod0)
  ...
  emval1=arma_diffem_N(PC,distC,ndata,lambdas,mod0)
  ...
  # emval2[lit]=diffem(PC,distC,ndata,lambdas2[lit],mod0)
  # emval2[lit]=rcpp_diffem(PC,distC,ndata,lambdas2[lit],mod0)
  
  # }
  
  emval2=arma_diffem_N(PC,distC,ndata,lambdas2,mod0)
  ...
  #emval3[lit+2]=diffem(PC,distC,ndata,lambdas3[lit],mod0)
  #emval3[lit+2]=rcpp_diffem(PC,distC,ndata,lambdas3[lit],mod0)
  #emval3[lit+2]=rcpp_diffem(PC,distC,ndata,lambdas3[lit],mod0)
  #}
  
  emval3=arma_diffem_N(PC,distC,ndata,lambdas3,mod0)
...
locmodes=function(data,mod0=1,lowsup=-Inf,uppsup=Inf,n=2^15,tol=10^(-5),display=F,...){
  ...
  #Posiciones en las que est谩n los m谩ximos
  z=c(1:(n-1))
  re=z[diff(fn$y)>0]
  z2=c(1:length(re))
  se=z2[diff(re)>1]
...
#####
dyn.load("/public/software/apps/R/4.2.3/lib64/R/library/multimode/libs/multimode.so")
# dlls<-getLoadedDLLs()
#getDLLRegisteredRoutines(dlls[["multimode"]])
#is.loaded("calcdist")

cl <- makeCluster(100)#

clusterEvalQ(cl,library("multimode"))
clusterEvalQ(cl,library("Rcpp"))
clusterEvalQ(cl,sourceCpp('./Rcpp_FUNC.cpp'))
clusterEvalQ(cl,sourceCpp('./RcppArma_FUNC.cpp'))
clusterExport(cl,varlist=c("modetest","emcbw","dip","excessmass","excessmassex","excessmassapp","diffem"),envir=environment())


# Set Parameters series
True_mean_S <- 10
True_sd_S <- seq(from = 1,to = 5,by = 1)
True_mean_L <- seq(from = 10,to = 20,by = 0.1)
True_sd_L <- seq(from = 1,to = 5,by = 1)
True_mean_L/True_mean_S - 1
Pop_ratio <- seq(from = 1,to = 5,by = 0.1)
# Skewness_gamma_1 <- seq(-0.95,0.7,by = 0.05)
# Skewness_gamma_2 <- Skewness_gamma_1 + 0.25
Abs_skew <- 0
Skew_diff <- seq(from = -1.9,to = 1.9,by = 0.025)

Skew_com <- 
  tibble(Abs_skew_col = rep(Abs_skew,times = length(Skew_diff)),
         Skew_diff_col = rep(Skew_diff,each = length(Abs_skew))) %>% 
  mutate(Skewness_gamma_1_col = Abs_skew_col - (Skew_diff_col/2),
         Skewness_gamma_2_col = Abs_skew_col + (Skew_diff_col/2),
         .before = Abs_skew_col)
...
# Define the function
Modetest_1<-function(Vect){
  ACR<-modetest(Vect,mod0 = 1,method = "ACR",B = 500,
                submethod = NULL,n = NULL,tol = NULL,tol2 = NULL,gridsize = NULL)
  return(c(Pval = ACR[["p.value"]]))
}
Modetest_2<-function(Vect){
  ACR<-modetest(Vect,mod0 = 2,method = "ACR",B = 500,
                submethod = NULL,n = NULL,tol = NULL,tol2 = NULL,gridsize = NULL)
  return(c(Pval = ACR[["p.value"]]))
}
Modetest_3<-function(Vect){
  ACR<-modetest(Vect,mod0 = 3,method = "ACR",B = 500,
                submethod = NULL,n = NULL,tol = NULL,tol2 = NULL,gridsize = NULL)
  return(c(Pval = ACR[["p.value"]]))
}
# Modetest_1<-function(Vect){
#   D<-dip.test(Vect, simulate = TRUE, B = 1000)
#   return(c(Pval = D[["p.value"]]))
# }
...
# Build dataset following simulated data distribution
set.seed(123)

# Parameters setting
# Group 1:length(Pop_ratio)
M1 = 1;True_mean_S[M1]
# M2 = 1;True_mean_L[M2]
...
Function_skewness_alpha <- 
  function(alpha,skewness_gamma_1) ((4 - pi)/2)*(((alpha/((1+alpha^2)^0.5)) * (2/pi)^0.5)^3)/((1 - 2*((alpha/((1+alpha^2)^0.5))^2)/pi)^1.5) - skewness_gamma_1
...
for (M2 in 85:101) {
  ...
  for (Row_num in 1:nrow(Skew_com)) {
    # Row_num = 10;M2 = 1 # Example
    Skewness_gamma_1 <- Skew_com$Skewness_gamma_1_col[Row_num]
    Skewness_gamma_2 <- Skew_com$Skewness_gamma_2_col[Row_num]
    Skew_diff <- Skew_com$Skew_diff_col[Row_num]
    
    # Create P_value tibble
    Pvalue_mod0_1 <- tibble(RepCout = 1:RepTimes)
    Pvalue_mod0_2 <- tibble(RepCout = 1:RepTimes)
    
    # calculate parameters in Skew normal distribution
    
    root_1 <- try(uniroot(Function_skewness_alpha,
                          tol = 0.000000000001,
                          skewness_gamma_1 = Skewness_gamma_1,
                          interval = c(-10,10)))
    alpha_1 = root_1$root
    omega_1 <- True_sd_S[S1]/((1 - 2*alpha_1^2/(pi*(1+alpha_1^2)))^0.5)
    xi_1 <- True_mean_L[M1] - ((2/(pi*(1+alpha_1^2)))^0.5)*omega_1*alpha_1
    
    root_2 <- try(uniroot(Function_skewness_alpha,
                          tol = 0.000000000001,
                          skewness_gamma_1 = Skewness_gamma_2,
                          interval = c(-10,10)))
    alpha_2 = root_2$root
    omega_2 <- True_sd_L[S2]/((1 - 2*alpha_2^2/(pi*(1+alpha_2^2)))^0.5)
    xi_2 <- True_mean_L[M2] - ((2/(pi*(1+alpha_2^2)))^0.5)*omega_2*alpha_2
    
    # j = Set the sample size for each sample
    # length(sample_size)——循环次数
    for (j in 1:length(sample_size)) {
      # for (j in 1:3){
      
      print(paste0('SampleSize = ',sample_size[j]))
      
      # Create an empty tibble with the desired number of rows
      Sample_tibble <- tibble(Id = 1:sample_size[j])
      
      # Use a loop to add the desired number of columns
      # i = number of columns=times of repetition=Set the number of samples to generate
      for (i in 1:RepTimes) {
        # Create column names using paste and seq
        colname_i <- paste0("repeat counter ",i)
        
        # Generate simulated data from the selected mean and standard deviation
        Simulated_Pop1 <- rsn(n = ceiling(sample_size[j]/(1+Pop_ratio[PR])), 
                              xi = xi_1, omega = omega_1, alpha = alpha_1)
        Simulated_Pop2 <- rsn(n = sample_size[j]-ceiling(sample_size[j]/(1+Pop_ratio[PR])),
                              xi = xi_2, omega = omega_2, alpha = alpha_2)
        # c(Simulated_Pop2)
        # Simulated_Pop2 <- rnorm(n = sample_size[j]-ceiling(sample_size[j]/(1+Pop_ratio[PR])), 
        #                         mean =True_mean_L[M2], sd = True_sd_L[S2])
        
        # Combine the two sets of simulated data into one vector
        Sample_tibble <- Sample_tibble %>% 
          add_column(!!colname_i :=c(Simulated_Pop1,Simulated_Pop2))
      }
      colname_j <- paste0("Samp size ",sample_size[j])
      if(Test_on_mod0_1){
        
        # Pvalue_mod0_1 <- Pvalue_mod0_1 %>%
        # add_column(!!colname_j :=map_dfr(Sample_tibble[,-1],Modetest_1)$Pval)
        Pvalue_mod0_1 <- Pvalue_mod0_1 %>%
          add_column(!!colname_j := parApply(cl, as.matrix(Sample_tibble[,-1]),2,Modetest_1))
      }
      if(Test_on_mod0_2){
        
        # Pvalue_mod0_2 <- Pvalue_mod0_2 %>%
        # add_column(!!colname_j :=map_dfr(Sample_tibble[,-1],Modetest_2)$Pval)
        Pvalue_mod0_2 <- Pvalue_mod0_2 %>%
          add_column(!!colname_j := parApply(cl, as.matrix(Sample_tibble[,-1]),2,Modetest_2))
      }
    }
    
    
    # Output Pvalue matrix
    Pvalue <- list(Pvalue_mod0_1,Pvalue_mod0_2) %>% 
      set_names(c("Pvalue_mod0_1","Pvalue_mod0_2"))
    
    Filename <- paste0("DI_",round(True_mean_L[M2]/True_mean_S[M1]-1,3),
                       "_SD_S_",round(True_sd_S[S1],3),
                       "_SD_L_",round(True_sd_L[S2],3),
                       "_Ratio_",round(Pop_ratio[PR],3),
                       "_Abs_skew_",round(Abs_skew,3),
                       "_Skew_diff_",round(Skew_diff,3),
                       ".xlsx")
    
    # Filename <- paste0('./RWorkspace/',Filename)
    
    write_xlsx(Pvalue,path = Filename)
    
    #write_csv(Pvalue,file = Filename)
  }
}
...
timeEnd<-Sys.time()
difftime(timeEnd,timeBegin,units="secs")
