...
for (M2 in 49:60){
  ...
  for (SK in 1:length(Skewness_gamma)) {
    ...
    root <- try(uniroot(Function_skewness_alpha,
                         tol = 0.000000000001,
                         skewness_gamma_1 = Skewness_gamma[SK],
                         interval = c(-10,10)))
    alpha = root$root
    omega_1 <- True_sd_L[S1]/((1 - 2*alpha^2/(pi*(1+alpha^2)))^0.5)
    xi_1 <- True_mean_L[M1] - ((2/(pi*(1+alpha^2)))^0.5)*omega_1*alpha
    omega_2 <- True_sd_L[S2]/((1 - 2*alpha^2/(pi*(1+alpha^2)))^0.5)
    xi_2 <- True_mean_L[M2] - ((2/(pi*(1+alpha^2)))^0.5)*omega_2*alpha
    ...
    Filename <- paste0("DI_",round(True_mean_L[M2]/True_mean_S[M1]-1,3),
                       ...
                       "_Skew_",round(Skewness_gamma[SK],3),
                       ".xlsx")
