...
for (M2 in 61:72){
  ...
  for (SK in 1:length(Skewness_gamma)) {
    ...
    Function_skewness_alpha <- function(alpha,skewness_gamma_1) ((4 - pi)/2)*(((alpha/((1+alpha^2)^0.5)) * (2/pi)^0.5)^3)/((1 - 2*((alpha/((1+alpha^2)^0.5))^2)/pi)^1.5) - skewness_gamma_1
    root <- try(uniroot(Function_skewness_alpha,
                        tol = 0.000000000001,
                        skewness_gamma_1 = Skewness_gamma[SK],
                        interval = c(-10,10)))
    alpha = root$root
    omega <- True_sd_L[S2]/((1 - 2*alpha^2/(pi*(1+alpha^2)))^0.5)
    xi <- True_mean_L[M2] - ((2/(pi*(1+alpha^2)))^0.5)*omega*alpha
    ...
    for (j in 1:length(sample_size)) {
      ...
      for (i in 1:RepTimes) {
        ...
        Simulated_Pop1 <- rnorm(n = ceiling(sample_size[j]/(1+Pop_ratio[PR])), 
                                mean =True_mean_S[M1], sd = True_sd_S[S1])
