...
SD_com <- 
  tibble(True_sd_S_col = rep(True_sd_S,times = length(SD_ratio)),
         SD_ratio_col = rep(SD_ratio,each = length(True_sd_S))) %>% 
  mutate(True_sd_L_col = True_sd_S_col*SD_ratio_col,
         .before = SD_ratio_col)
...
for (M2 in 13:24){
  ...
  for (Row_num in 1:nrow(SD_com)) {
    ...
    Filename <- paste0("DI_",round(True_mean_L[M2]/True_mean_S[M1]-1,3),
                       "_SD_S_",round(True_sd_S,3),
                       "_SD_L_",round(True_sd_L,3),
