...
True_mean_S <- 10
# True_sd_S <- seq(from = 0.1,to= 5,by = 0.1)
True_mean_L <- seq(from = 10,to = 20,by = 0.1)
# True_sd_L <- seq(from = 0.1,to= 5,by = 0.1)
Abs_SD <- 1.5
SD_ratio <- seq(from = 1,to = 5,by = 0.1)
...
SD_com <- 
  tibble(Abs_SD_col = rep(Abs_SD,times = length(SD_ratio)),
         SD_ratio_col = rep(SD_ratio,each = length(Abs_SD))) %>% 
  mutate(True_sd_S_col = Abs_SD_col/sqrt(SD_ratio_col),
         True_sd_L_col = Abs_SD_col*sqrt(SD_ratio_col),
         .before = Abs_SD_col)
...
for (M2 in 61:72) {
  ...
  for (Row_num in 1:nrow(SD_com)) {
    # Row_num = 10;M2 = 1 # Example
    True_sd_S <- SD_com$True_sd_S_col[Row_num]
    True_sd_L <- SD_com$True_sd_L_col[Row_num]
    SD_ratio <- SD_com$SD_ratio_col[Row_num]
    ...
    Filename <- paste0("DI_",round(True_mean_L[M2]/True_mean_S[M1]-1,3),
                       "_Abs_SD_",round(Abs_SD,3),
                       "_SD_R_",round(SD_ratio,3),
